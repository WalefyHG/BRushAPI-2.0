...
from ninja import UploadFile, File
...
class NoticesController:
    ...
    def get_notices(self, request):
        ...
        for notice in notices:
            notice_out = NoticeOut(
                ...
                notice_image=notice.notice_image.url,
                notice_writer=notice.notice_writer.user_name
            )
            serialized_notices.append(notice_out)
        return serialized_notices
    
    @route.post('/notices', response={200: NoticeOut})
    def create_notices(self, request, notices: NoticeIn, image: UploadFile = File(None)):
        user = request.auth
    ...
    def update_notices_by_id(self, request, id: int, notice: NoticeIn, image: UploadFile = File(None)):
        user = request.auth
